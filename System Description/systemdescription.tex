...
\item The REST API is reachable only over HTTPS, in accord with \emph{1. Simplicity} and \emph{2. Open Design} since no custom protocol is used. Ideally client side verification is used to ensure that only the Webserver
can send requests (\emph{8. Complete Mediation}). 
\end{itemize}
...
\item As for \emph{b. Secrecy and integrity with respect to the private keys in the key backup}, a copy of the key is encrypted, using the public key of a dedicated recovery key pair, before being backed-up
\end{itemize}
% Describe the system's security design, including access control, key and session management,  and security of data at rest and in transit.
...
%Sadly the library used to program it doesn't yet support client side verification over HTTPS, so anyone in the internal network can make requests to the REST API.
...
\noindent We did this with netcat listening on port 9844 for 10 seconds:
\begin{verbatim}
...
\hline
1 & Nature: Floods, lightnings, fire & Building has necessary physical countermeasures & {\it Low} & {\it Med} & {\it Low} \\
\hline
