...
The Core CA server runs in the iMovies internal network at IP address 192.168.50.31 and exposes a SparkJava REST API on port 8100, which accepts only HTTPS connections and uses a certificate signed with the CA root key. Sadly the library doesn't yet support client side certificate verification, therefore only the server side is verified. The API offers calls to issue and revoke certificates, as well as to get information about the state of the CA.\\
...
The Database server runs in the iMovies internal network at IP address 192.168.50.33 and exposes a SparkJava REST API on port 8100, which accepts only HTTPS connections and uses a certificate signed with the CA root key. Sadly the library doesn't yet support client side certificate verification, therefore only the server side is verified. The API offers calls to handle user data.\\
