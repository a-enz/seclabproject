...
\begin{itemize}
\item In order to obtain a new certificate, the user in question has to authenticate themselves to the web server. This is either done via entering
user credentials (user ID/password) or by providing a client certificate issued by the iMovies CA. The user can then choose to obtain a new certificate
upon which they are shown what first name, last name and email address is stored under their user ID in the database. If desired, the user can make 
changes to these data. Additionally, they have to provide a password, with which the new certificate/private key bundle will be encrypted. 
\end{itemize}
