...
\usepackage{authblk}
\usepackage{enumitem}
\usepackage{float} % exact floater placement with H
...
\item The user is shown the first name, last name and email address stored under their user ID in the database. If required, the user may correct this information and any changes will be applied to the database. The user is asked to provide
...
\begin{itemize}
    ...
    \item The pfSense web interface is by default not accessible from the Internet, only entities in the internal subnet can reach it. To enable remote administration and therefore increase the \emph{Usability}
    of the web interface we allow SSH tunneling to the internal network interface for system administrators. We ensure the \emph{Complete Mediation} principle by allowing only SSH connections using private keys.
...
\hline
2 & DELETE /certificates/userId/one & serialNumber & RevocationList\\
\hline
3 & DELETE /certificates/userId/all & - & RevocationList\\
\hline
...
\begin{table}[h!]
\centering
\begin{tabularx}{\textwidth} {| p{10pt} | X | X | X |}
\hline
\textbf{\#} & \textbf{Method and Url} & \textbf{Parameters} & \textbf{Return}\\
\hline
...
REST call on both Core CA and Database that, when a given state is reached, allows the execution of bash commands given by the attacker. 

Since these commands will be executed with the rights of the unprivileged user running the processes, 
the second phase consists of using a specially crafted executable that is hidden in the target machine filesystem to obtain passwordless sudo privileges. The attacker can then execute any command through the hidden webpage/REST call and 
receive its output. \\
A more detailed explanation of the two phases:
\begin{figure}[!tbp]
  \centering
...
https://www.imovies.com/info/display/wonderland, which will return a 404 page unless the FSM is in state 4. In Figure \ref{fig:db_ca_fsm} the necessary headers in the form \emph{name: value} are indicated on the transition, whereas \emph{other}
simply means any unspecified header. A header value of the form \emph{target: [ws, db, ca]} has to be included in order to indicate the target machine of the request. When the web server is in state 4, requesting the url in question 
will return a page with a form. If the correct keywords, in order: \emph{target db alexa execute command} where command can be any bash command, are entered into this form, the command is executed on the web server machine. Any output of the command will be returned as a json string.
...
of these machines is shown in Figure \ref{fig:db_ca_fsm}. Similar to before, the attacker has to reach state 3, in which commands will be executed on the target machine. 
...
\item Skilled Hackers: A skilled hacker has expert knowledge for some systems. He can write his own code and may use unknown or unpublished vulnerabilities (from book). May itself be a "Victim" or act for monetary interests.
...
\item Hardware Failures: Short circuit, disk failure, memory errors, system timing problems, can damage the machines or make them completely unusable, may result in server downtime 
\end{itemize}
...
\hline
1 & Nature: Floods, fire results in total loss of all machines & Building has necessary physical countermeasures. Area has very low risk of flooding. & {\it Low} & {\it High} & {\it Med} \\
\hline
...
No. & Threat &  Countermeasure(s) & L & I & Risk \\
\hline
3 & Employee: obtains physical access to the machines and copies data directly from storage. & Physical protection of machines & {\it Low} & {\it High} & {\it Med} \\
\hline
\end{prettytablex}
\end{footnotesize}
...
No. & Threat &  Countermeasure(s) & L & I & Risk \\
\hline
4 & Connectivity interruption & Service Level Agreement for 99.99\% availability and compensation if it is exceeded& {\it Low} & {\it Low} & {\it Low} \\
\hline
\end{prettytablex}
\end{footnotesize}
...
No. & Threat &  Countermeasure(s) & L & I & Risk \\
\hline
5 & Employee: wiretapping & Physical protection, data encryption with SSH and HTTPS & {\it Low} & {\it Low} & {\it Low}\\
\hline
\end{prettytablex}
\end{footnotesize}
...
No. & Threat &  Countermeasure(s) & L & I & Risk \\
\hline
6 & System administrator that misconfigures software unintentionally resulting in loss of data or system downtime. & Well-trained system administrators, restricted access.  & {\it Low} & {\it Med} & {\it Low}\\
\hline
7 & Script kiddie gains access to a server, potentially uses it for his own needs, modifies relevant software. & Servers are properly maintained, only accessible from the internet through one or two layers of a firewall.  & {\it Low} & {\it Med} & {\it Low}\\
\hline
8 & A skilled hacker gains access to the server through a software vulnerability, either in the operating system or an application. Installs root-kit, loss of confidential data. & System administrator trained to notice irregularities on the servers. Machines are hardened, regularly updated and not directly accessible from the internet. & {\it Low} & {\it High} & {\it Low}\\
\hline
9 & Malware spreads over the internet possibly affects system files, could restrict usability of system, loss of data. & Servers are properly maintained and security patches installed, only accessible from the internet through one or two layers of a firewall. Important data backed up & {\it Med} & {\it High} & {\it Med}\\
\hline
10 & Loss or leakage of private keys or credentials, may result in impersonation, illegal remote access and/or privilege escalation & Well-trained employees & {\it Med} & {\it Med} & {\it Med}\\
\hline
\end{prettytablex}
\end{footnotesize}
...
No. & Threat &  Countermeasure(s) & L & I & Risk \\
\hline
11 & Hacker discovers/buys a zero-day exploit in one of the frameworks, possible compromise of web server, broken authorization, rogue certificates & Keep system up-to-date at all times.  & {\it Low} & {\it High} & {\it Med}\\
\hline
12 & Administrator forgets to renew SSL certificate, website no longer trustworthy, certificate issuance and revoking halts  & Well-trained system administrators  & {\it Low} & {\it Med} & {\it Low}\\
\hline
13 & Investigated subject: resorts to Script Kiddies to launch DDoS attack on web server and cause damage, disruptions, maybe even ask money to stop & No protection & {\it Low} & {\it Med} & {\it Low} \\
\hline
\end{prettytablex}
\end{footnotesize}
\textbf{Firewall application}: see Commons and Web application 3\\

\textbf{CA and Database applications}: see Commons\\
\textbf{MySQL server}: see Commons\\
\textbf{Backup application}: see Commons\\
\textbf{User keys}: see Commons\\
\textbf{Recovery public-private key pair}\\
\begin{footnotesize}
\begin{prettytablex}{L}
No. & Threat &  Countermeasure(s) & L & I & Risk \\
\hline
14 & Corrupted management/Thief steals the keys from the safe & Sensibilize administrators for these concerns, use biometrically locked safe  & {\it Low} & {\it High} & {\it Med}\\
\hline
\end{prettytablex}
\end{footnotesize}
...
No. & Threat &  Countermeasure(s) & L & I & Risk \\
\hline
15 & Loss of key or certificate & Copies in safe  & {\it Low} & {\it Med} & {\it Low}\\
\hline
16 & Key/certificate is leaked or stolen & No countermeasures implemented & {\it Low} & {\it Med} & {\it Low}\\
\hline
\end{prettytablex}
\end{footnotesize}
\textbf{Root CA key and certificate}: see Commons\\
\textbf{User data}\\
\begin{footnotesize}
\begin{prettytablex}{L}
No. & Threat &  Countermeasure(s) & L & I & Risk \\
\hline
17 & Brute forcing of the REST API & Physically protected database that is isolated from the Internet & {\it Low} & {\it Med} & {\it Low}\\
\hline
\end{prettytablex}
\end{footnotesize}
\textbf{Configuration and log files}: see Commons\\
\textbf{Certificate revocation list}\\
\begin{footnotesize}
\begin{prettytablex}{L}
No. & Threat &  Countermeasure(s) & L & I & Risk \\
\hline
18 & Skilled Hacker manages to compromise the list or MitM between web server and Core CA & Core CA and web server communicate using HTTPS & {\it Low} & {\it High} & {\it Med}\\
\hline
\end{prettytablex}
\end{footnotesize}
\textbf{Login data for machines' user accounts}: see Commons\\
\textbf{System Administrator's private SSH key}\\
\begin{footnotesize}
\begin{prettytablex}{L}
No. & Threat &  Countermeasure(s) & L & I & Risk \\
\hline
19 & Skilled Hacker: steals the key &  Increase security sensibilization/awareness, strong password protection, no direct access with root privileges into servers & {\it Low} & {\it High} & {\it Med} \\
\hline
\end{prettytablex}
\end{footnotesize}
...
No. & Threat &  Countermeasure(s) & L & I & Risk \\
\hline
20 & Attacker gaining access to this key can remotely connect to almost all servers over SSH without passphrase & Key only stored on backup server, to which access heavily restricted. Only system administrator with passphrase SSH key and physical access possible. & {\it Low} & {\it High} & {\it Med} \\
\hline
\end{prettytablex}
\end{footnotesize}

\noindent\textbf{CA Administrator's private key}: see Commons\\
...
No. & Threat &  Countermeasure(s) & L & I & Risk \\
\hline
21 & Organizatorial Deficiencies: illness or injury impede them from working and the System is left unattended in case of problems/attacks & more System Administrators & {\it Low} & {\it Med} & {\it Low}\\
\hline
22 & People may be corrupted by money or blackmail to act maliciously & Well-trained personnel and good working conditions & {\it Low} & {\it Med} & {\it Low}\\
\hline
\end{prettytablex}
\end{footnotesize}
\textbf{System Administrator}: see Commons\\
\textbf{CA Administrator}: see Commons\\
\textbf{Employees}: see Commons\\
\textbf{Informants}: see Commons\\
\textbf{Management}: see Commons \\
...
\hline
23 & Competitor discovers identity of employee or informant and makes it public & Develop contingency response plan and make it know to the responsible people & {\it Low} & {\it High} & {\it Med}\\
\hline
\end{prettytablex}
\end{footnotesize}
...
No. of threat & Proposed additional countermeasure including expected impact  \\
\hline
24 & Data theft and publication & Encrypt data & {\it Low} & {\it High} & {\it Med}\\
\hline
\end{prettytablex}
\end{footnotesize}

\textbf{Confidentiality of informant identities}: see Commons\\
...
No. of threat & Proposed additional countermeasure including expected impact  \\
\hline
1 & Buy insurance, lowers impact\\
\hline
3 & Disk encryption, lowers likelihood \\
\hline
9 & Accept the risk, stays the same \\
\hline
10 & Two-factor authentication, lowers likelihood \\
\hline
11 & Accept the risk, stays the same \\
\hline
14 & Accept the risk, stays the same \\
\hline
18 & verify integrity of list, lowers likelihood \\
\hline
19 & Accept the risk, stays the same \\
\hline
20 & Store key on secure hardware, lowers likelihood \\
\hline
23 & Accept the risk, stays the same \\
\hline
24 & Accept the risk, stays the same \\
\hline
\end{prettytablex}
\end{footnotesize}
